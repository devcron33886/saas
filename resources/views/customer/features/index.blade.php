        ...
        <div class="col-md-12">
            @can('feature_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('customers.features.create') }}">
                                ...
                                @foreach($features as $key => $feature)
                                    <tr data-entry-id="{{ $feature->id }}">
                                        ...
                                        <td>
                                            ...
                                            @can('feature_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('customers.features.edit', $feature->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('feature_delete')
                                                <form action="{{ route('customers.features.destroy', $feature->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('feature_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('customers.features.massDestroy') }}",
