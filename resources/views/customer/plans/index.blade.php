        ...
        <div class="col-md-12">
            @can('plan_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('customers.plans.create') }}">
                            ...
                            <tbody>
                                @foreach($plans as $key => $plan)
                                    <tr data-entry-id="{{ $plan->id }}">
                                        ...
                                        <td>
                                            <span style="display:none">{{ $plan->status ?? '' }}</span>
                                            <input type="checkbox" disabled="disabled" {{ $plan->status ? 'checked' : '' }}>
                                        </td>
                                        <td>

                                            @can('plan_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('customers.plans.edit', $plan->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('plan_delete')
                                                <form action="{{ route('customers.plans.destroy', $plan->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('plan_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('customers.plans.massDestroy') }}",
