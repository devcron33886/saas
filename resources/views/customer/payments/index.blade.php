        ...
        <div class="col-md-12">
            @can('payment_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('customers.payments.create') }}">
                            ...
                            <tbody>
                                @foreach($payments as $key => $payment)
                                    <tr data-entry-id="{{ $payment->id }}">
                                        ...
                                        <td>
                                            @can('payment_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('customers.payments.show', $payment->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('payment_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('customers.payments.edit', $payment->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('payment_delete')
                                                <form action="{{ route('customers.payments.destroy', $payment->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('payment_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('customers.payments.massDestroy') }}",
