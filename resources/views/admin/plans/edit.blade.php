    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.plans.update", [$plan->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('stripe_identifier') ? 'is-invalid' : '' }}" type="text" name="stripe_identifier" id="stripe_identifier" value="{{ old('stripe_identifier', $plan->stripe_identifier) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('billing_period') ? 'is-invalid' : '' }}" type="text" name="billing_period" id="billing_period" value="{{ old('billing_period', $plan->billing_period) }}" required>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('status') ? 'is-invalid' : '' }}">
                    <input class="form-check-input" type="checkbox" name="status" id="status" value="1" {{ $plan->status || old('status', 0) === 1 ? 'checked' : '' }} required>
